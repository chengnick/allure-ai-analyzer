...
def _call_gemini(user_prompt: str, api_key: str, max_retries: int = 3) -> str:
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if resp.status_code in (429, 500, 502, 503) and attempt < max_retries:
            ...
            print(f"[classifier] HTTP {resp.status_code},{wait}s 後重試 ({attempt}/{max_retries})")
            time.sleep(wait)
            continue
        ...
        try:
            ...
            raise RuntimeError(f"Gemini 回應結構異常: {json.dumps(data)[:500]}") from e
...
if __name__ == "__main__":
    ...
    sample = [
        {
            "test_name": "test_login[brandA]",
            "status": "broken",
            "message": "requests.exceptions.ConnectionError: HTTPSConnectionPool(host='api.brand-a.example', port=443)",
            "trace": "Traceback (most recent call last):\n  File \"auth.py\", line 42, in login\n    resp = session.post(url, ...)\nrequests.exceptions.ConnectionError: ...",
        },
        {
            "test_name": "test_get_balance[brandB]",
            "status": "failed",
    ...
    for f, r in zip(sample, classify_failures(sample)):
        print(f"{f['test_name']}: {r['category']} ({r['confidence']}) — {r['reasoning']}")
